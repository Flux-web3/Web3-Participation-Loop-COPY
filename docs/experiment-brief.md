...
- **Reproducibility.** Simulation is seeded and replayable; every metric is derived from the labelled append-only synthetic event log; results will be recorded against labelled records, not asserted from memory. Prototype for reference: https://web3-participation-loop.vercel.app
