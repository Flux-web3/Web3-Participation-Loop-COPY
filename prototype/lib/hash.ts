...
/**
 * Pure-TS SHA-256 (FIPS 180-4), byte-identical hex output to node:crypto's
 * `createHash('sha256').digest('hex')` so hashes are deterministic and identical
 * in tests (Node), the seed, and the browser build — `node:crypto` is not
 * available in the Vite client bundle.
 */
